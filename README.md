...
Data Preparation: The dataset, which contains comments labeled as toxic or non-toxic, is preprocessed and prepared for training.
Model Fine-Tuning: A pre-trained BERT model is fine-tuned on the prepared dataset. This process adjusts the model parameters to better fit the specific characteristics of Tunisian dialect toxicity.
Evaluation: The model's performance is evaluated to ensure it accurately detects toxic comments. The trained model weights are saved for deployment.
Model Deployment
